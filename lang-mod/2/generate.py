...
# data
flags.DEFINE_string('load_model',   path + 'cv/epoch024_5.1917.model',    'filename of the model to load')
# we need data only to compute vocabulary
flags.DEFINE_string('data_dir',   path + '/data',    'data directory')
flags.DEFINE_integer('num_samples', 600, 'how many words to generate')
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        rnn_state = session.run(m.initial_rnn_state)
        f = open(path + 'generated_lang.txt', 'w+')
